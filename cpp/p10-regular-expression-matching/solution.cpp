...
public:
    ...
    static bool dfs(const string &s, const string &p, int i, int j) {
        ...
        if ((s[i] == p[j] || p[j] == '.') && dfs(s, p, i - 1, j - 1)) return true;
        return p[j] == '*' &&
               (
                   (s[i] == p[j - 1] || p[j - 1] == '.') &&
    ...
    // 动态规划 - 正确解法
    //
    // 题目中的匹配是一个「逐步匹配」的过程：我们每次从字符串 p 中取出一个字符或者「字符 + 星号」的组合，并在 s 中进行匹配。
    // 对于 p 中一个字符而言，它只能在 s 中匹配一个字符，匹配的方法具有唯一性；而对于 p 中字符 + 星号的组合而言，
    ...
    //                    │                    ╰ false,       otherwise
    //          f[i][j] = ┤
    //                    │             ╭ f[i-1][j] || f[i][j-2], matches(s[i], p[j-1])
    //                    │ otherwise = ┤
    //                    ╰             ╰ f[i][j-2],              otherwise
    //
    ...
    //
    // 在上面的状态转移方程中，如果字符串 p 中包含一个「字符 + 星号」的组合（例如 a*），那么在进行状态转移时，会先将 a 进行匹配
    // （当 p[j] 为 a 时），再将 a* 作为整体进行匹配（当 p[j] 为 * 时）。
    // 然而，在题目描述中，我们必须将 a* 看成一个整体，因此将 a 进行匹配是不符合题目要求的。
    //
    // 思考：看来我们进行了额外的状态转移，这样会对最终的答案产生影响吗？
    static bool is_match(const string &s, const string &p) {
